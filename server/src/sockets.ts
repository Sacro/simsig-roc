...
export function rocSockets(socket: Socket, gameManager: ROCManager, callManager: CallManager) {
  ...
  socket.on('moveToLobby', async () => {
    await gameManager.movePlayerToLobby(socket.id)
  })

  socket.on('markAFK', async () => {
    await gameManager.markPlayerAFK(socket.id)
  })

  socket.on('updatePlayerPanel', (msg: CallRequest) => {
    gameManager.updatePlayerPanel(msg.user, msg.panel)
  })

  socket.on('claimPanel', function (msg: { sender: string, sim: string, panel: string }) {
