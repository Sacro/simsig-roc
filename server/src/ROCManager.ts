...
export default class ROCManager {
  ...
  activateGame(game: {
    ...
    // TODO: change to correct default port?
    this.stompManager.createClientForGame(game, game.interfaceGatewayPort ?? 50505)
  ...
  enableInterfaceGateway(simId: string) {
    this.stompManager.activateClientForGame(simId)
    this.updateAdminUI()
  }

  disableInterfaceGateway(simId: string) {
    this.stompManager.deactivateClientForGame(simId)
    this.updateAdminUI()
  }

  enableConnections(simId: string) {
    const sim = this.getSimById(simId)
    if (sim) {
      sim.connectionsOpen = true
      this.updateAdminUI()
    }
  }
  ...
  async registerWebUI(socket: Socket, discordId: string) {
