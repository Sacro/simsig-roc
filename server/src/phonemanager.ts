...
export class PhoneManager {
  ...
  generatePhoneForTrain(train: Train) {
  ...
  generatePhoneForPanel(sim: Simulation, panel: Panel) {
    const phone = new Phone(sim.id + '_' + panel.id, sim.name + ' ' + panel.name, Phone.TYPES.FIXED, new Location(sim.id, panel.id))
    this.phones.push(phone)
    return phone
  }

  generatePhoneForPerson(number: string,
    name: string,
    type: string = Phone.TYPES.MOBILE,
  ...
  getRECRecipientsForPhone(phone: Phone) {
    ...
    const panel: Panel | undefined | null = sim?.getPanel(phone.getLocation()?.panelId)
